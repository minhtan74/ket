// Trang danh sách đề Part 7. Tương đương AdminPart7Controller::index().
import AdminNavTabs from '@/components/AdminNavTabs';
import AdminFlash from '@/components/AdminFlash';
import DeleteButton from '@/components/DeleteButton';
import { getAllTests } from '@/lib/models/part7';
import { deleteTestAction } from './actions';

export const metadata = { title: 'Quản trị - KET Reading Part 7' };

export default async function AdminPart7Page({ searchParams }) {
    ...
    <>
      ...
      <main className="page">
        ...
        <section className="admin-section">
          ...
          <p>
            <a href="/admin/part7/test/new" className="btn-link btn-add">
              + Thêm đề mới
            </a>
          </p>
        </section>
              ...
              <tbody>
                {testsInGroup.map((t) => (
                  <tr key={t.id}>
                    ...
                    <td>
                      {t.title}
                      {t.data_note ? <span title="Có ghi chú dữ liệu suy luận"> ⚠</span> : null}
                    </td>
                    <td>{t.letter_count}</td>
                    <td>{t.question_count}</td>
                    <td className="admin-actions">
                      <a href={`/admin/part7/manage/${t.id}`}>Quản lý</a>
                      <a href={`/admin/part7/test/${t.id}`}>Sửa</a>
                      <form action={deleteTestAction.bind(null, t.id)}>
                        <DeleteButton message="Xoá cả đề này (gồm toàn bộ thư và câu hỏi)?" />
                      </form>
                    </td>
                  </tr>
                ))}
